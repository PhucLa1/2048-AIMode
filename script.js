...
function initializeGame() {
    boardReal = [...Array(size)].map(e => Array(size).fill(0));
    placeRandom(boardReal);
    placeRandom(boardReal);
    renderBoard();
}
...
function renderBoard() {
    for (let i = 0; i < size; i++) {
        for (let j = 0; j < size; j++) {
            ...
            const currentValue = boardReal[i][j];
            if (currentValue !== 0) {
...
function placeRandom(board,option) {
    const available = [];
    for (let i = 0; i < size; i++) {
        for (let j = 0; j < size; j++) {
            if (board[i][j] === 0) {
...
function move(board,direction,option) {
...
function transform(option,line, moveTowardsStart) {
...
function checkGameOver(board,option) {
    for (let i = 0; i < size; i++) {
        for (let j = 0; j < size; j++) {
            if (board[i][j] === 0) {
...
document.addEventListener('keydown', event => {
    if(gameRunning == true){
        if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(event.key)) {
            const out = move(boardReal, event.key, 1);
            //console.log(out);
        }
    }
});
